...
class AE(nn.Module):
    def __init__(self, embed_size, hidden_size, latent_size, encode_method, beta=0.1, learned_size=20, min_len=8, max_len=35, bidirectional=True):
        ...
        self.hidden2latent = nn.Linear(2 * hidden_size, latent_size).to(device)
    ...
    def forward(self, sequences):
        z = self.encode(sequences)
        
        amino_loss, acc = self.decode(sequences, z)
        
        return amino_loss, acc, z

    
    def encode(self, sequences):
        arrays, length = seq2num(sequences)

        batch_size = arrays.shape[0]
        
        h0 = torch.zeros(self.factor, batch_size, self.hidden_size).float().to(device)
        c0 = torch.zeros(self.factor, batch_size, self.hidden_size).float().to(device)
        
        peptide_encodings = torch.tensor(blosum_encode(arrays)).float().to(device)
    ...
    def decode(self, input_sequences, z):
        ...
        seq_arrays, length = seq2num(input_sequences)
        ...
        for t in range(self.max_len):
            ...
            for i, seq in enumerate(input_sequences):
                if t <= len(seq):
                    if t > 0: 
                        tokens.append(torch.tensor(encoded_arrays[i, t-1, :]).float().to(device))
            ...
            if t == 0:
                ...
                x_vecs = torch.stack(tokens, dim=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default='/users/PES0781/ziqichen/peptideproject/TCR/model2/reward/model/')
    parser.add_argument("--data_path", type=str, default="/users/PES0781/ziqichen/peptideproject/TCR/tcrdb/train_uniq_tcr_seqs.txt")
