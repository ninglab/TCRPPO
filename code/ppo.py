...
import io
import sys
from typing import Any, Dict, Optional, Iterable, Type, Union, Tuple, List
import pdb
import numpy as np
import torch as th
...
class PPO(OnPolicyAlgorithm):
    ...
    def pretrain(self, n_iter, batch_size, pairs) -> None:
        optimizer = optim.Adam(self.policy.parameters(), lr=self.learning_rate, amsgrad=True) 
        t1 = time.time()
        avg_loss = 0
        
        memories = []
        
        #batch_inputs = [[] for _ in len(self.env.remotes)]
        
        for i in range(n_iter):
            with torch.autograd.set_detect_anomaly(True):
                self.policy.zero_grad()
                
                loss = self.policy.pretrain(batch_size, pairs)
                loss.backward()
            ...
            if i > 0 and i % 50 == 0:
                t2 = time.time()
                print("time cost for iteration %d: %.4f, %.4f" % (i, t2-t1, avg_loss / 50))
                sys.stdout.flush()
                avg_loss = 0
    ...
    def _get_torch_save_params(self) -> Tuple[List[str], List[str]]:
        ...
        return state_dicts, ["policy_kwargs"]
    ...
    def load(
        ...
        **kwargs,
        ...
        data["policy_kwargs"]['features_extractor'] = pytorch_variables['features_extractor']
        ...
        if pytorch_variables is not None:
            for name in pytorch_variables:
                ...
                if pytorch_variables[name] is None or name == "features_extractor":
                    continue
    ...
    def save(
        ...
        for param_name in exclude:
            if param_name == "policy_kwargs":
                data[param_name] = {name: data[param_name][name] for name in data[param_name] if name != "features_extractor"}
            else:
        ...
        if torch_variable_names is not None:
            ...
            for name in torch_variable_names:
                if name == "policy_kwargs":
                    pytorch_variables["features_extractor"] = self.policy_kwargs['features_extractor']
                else:
