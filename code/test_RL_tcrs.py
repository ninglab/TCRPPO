...
class MySubprocVecEnv(VecEnv):
    ...
    #def reset(self, dones, alleles, obs) -> VecEnvObs:
    #    for remote, done, allele, ob in zip(self.remotes, dones, alleles, obs):
    #        remote.send(("reset", (done, allele, ob)))
    #    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tcrs", type=str)
    ...
    parser.add_argument('--peptide_path', type=str, default=cwd + "/ERGO/data/McPAS-TCR/peptides.txt")
    ...
    peptides = [peptide.strip() for peptide in open(args.peptides, 'r').readlines()]
    tcrs = [tcr.strip() for tcr in open(args.tcrs, 'r').readlines()]
    ...
    m_env_kwargs = {"action_space":action_space, "observation_space":observation_space, \
                "args": args, "max_tcr_len": args.max_len} #, "peptide_path": args.peptides} 
    ...
    rollout_peptides = [(peptide, tcr) for peptide in peptides for tcr in tcrs for _ in range(args.rollout)]
    ...
    while rollout < len(rollout_peptides):
        ...
        for idx, done in enumerate(dones):
            if done:
                peptide = rollout_peptides[batch_idxs[idx]][0]
